...
def parse_provider_names(raw_value: str | None) -> list[str]:
    from . import ALL_PROVIDER_NAMES, DEFAULT_PROVIDER_NAMES, PROVIDER_CLASS_BY_NAME
...
def build_cli_options(
    ...
    *,
    ...
    return CliOptions(
        ...
        overrides=merge_binary_overrides(group.overrides, subcommand_overrides),
    ...
    *,
    ...
    for dep in deps:
        ...
        if projected_abspath is not None:
            ...
            projected_abspath = projection_provider.project_binary(binary, dep_name)
    ...
    try:
        if update_before_run:
            ...
            try:
                ...
                loaded_for_update = None
            if loaded_for_update is not None:
...
def render_env_delta_values(
    ...
    return {key: changed_values[key] for key in ordered_keys}
...
def render_env_assignment_lines(
    ...
    *,
    ...
    changed_values = render_env_delta_values(base_env, final_env)
    return [
...
def env_command(
    ...
    **shared_kwargs: Any,
    ...
    """Emit env vars for the selected binaries or providers."""
...
def _normalize_binary_override_option_order(argv: list[str]) -> list[str]:
    ...
    Click only knows the binary-specific override flags on concrete binary
    subcommands, so a call like ``abxpkg --install-args '["black"]' upgrade
    black`` would normally fail at the group level before ``upgrade`` gets a
    chance to parse it. Hoist those flags across the first binary-managing
    subcommand so both placements normalize to the same argv.
    """
...
_ABXPKG_GROUP_OPTS_WITH_VALUES = (
    frozenset(
        opt
        for param in cli.params
        if isinstance(param, click.Option) and not param.is_flag
