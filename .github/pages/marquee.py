    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == "img" and any(name == "figure" for name, _ in self.stack):
            ...
            anchor = next(
                (
                    item["id"]
                    for name, item in reversed(self.stack)
                    if name in ("article", "figure") and item.get("id")
                ),
                "",
            )
            ...
            self.captures.append(
                {
                    ...
                    "href": self.base + "#" + anchor
                    if anchor
                    else urljoin(self.base, source),
                    "title": attrs["alt"],
